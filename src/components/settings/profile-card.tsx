...
export function ProfileCard({ profile, onSave }: { profile: DemoProfile; onSave: (profile: DemoProfile) => void }) {
  ...
  return (
    <section aria-labelledby="profile-title" className="min-w-0 rounded-2xl border border-outline bg-surface p-5 shadow-sm">
        ...
        <form onSubmit={saveProfile} className="mt-5 space-y-4">
          {fields.map((field) => (
            <div key={field.key}>
              ...
              <input id={`profile-${field.key}`} name={field.key} type={field.type} autoComplete={field.autocomplete} required maxLength={field.key === "email" ? 254 : 120} value={draft[field.key]} onChange={(event) => { setDraft({ ...draft, [field.key]: event.target.value }); setError(""); }} className="mt-2 min-h-11 w-full min-w-0 rounded-lg border border-outline bg-surface px-3 py-2 text-sm text-brand-dark" />
            </div>
          ))}
          ...
          <div className="flex flex-wrap justify-end gap-3 border-t border-outline pt-4">
            ...
            <button type="submit" className="min-h-11 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-on-brand hover:bg-brand-hover">Salvar perfil</button>
          </div>
        </form>
      </dialog>
    </section>
