...
export function PrivacyCard() {
  ...
  return (
    <section aria-labelledby="privacy-title" className="min-w-0 rounded-2xl border border-outline bg-surface p-5 shadow-sm">
      ...
      <dialog ref={dialogRef} aria-labelledby="manage-data-title" aria-describedby="manage-data-description" onClose={() => triggerRef.current?.focus()} className="fixed inset-0 m-auto max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-md overflow-y-auto rounded-2xl border border-outline bg-surface p-6 text-foreground shadow-xl backdrop:bg-black/40">
        ...
        <button type="button" onClick={() => dialogRef.current?.close()} className="mt-5 min-h-11 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-on-brand hover:bg-brand-hover">Fechar</button>
      </dialog>
    </section>
  );
}
