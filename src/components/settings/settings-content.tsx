...
import { appearanceOptions, initialNotifications, initialPreferences, initialProfile, notificationOptions, preferenceFields, type Appearance } from "@/data/settings";
...
export function SettingsContent() {
  ...
  return (
    <div className="mt-8 space-y-6">
      <p className="text-sm leading-6 text-muted">As opções são demonstrativas, valem apenas nesta página e são descartadas ao sair ou recarregar.</p>
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <ProfileCard profile={profile} onSave={setProfile} />
        <section aria-labelledby="preferences-title" className="min-w-0 rounded-2xl border border-outline bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {preferenceFields.map((field) => (
              <div key={field.id} className="min-w-0">
                ...
                <select id={`preference-${field.id}`} value={preferences[field.id]} onChange={(event) => { setPreferences({ ...preferences, [field.id]: event.target.value }); setFeedback(""); }} className="mt-2 min-h-11 w-full min-w-0 rounded-lg border border-outline bg-white px-3 py-2 text-sm text-brand-dark">
        ...
        <section aria-labelledby="notifications-title" className="min-w-0 rounded-2xl border border-outline bg-white p-5 shadow-sm">
        ...
        <section aria-labelledby="appearance-title" className="min-w-0 rounded-2xl border border-outline bg-white p-5 shadow-sm">
          ...
          <p className="mt-4 text-xs leading-5 text-muted">Seleção demonstrativa. A interface permanece no tema claro, inclusive ao selecionar Sistema.</p>
        </section>
      </div>
      <PrivacyCard />
      <div>
        <button type="button" onClick={() => {
          ...
          setFeedback("Preferências salvas localmente para esta demonstração. As escolhas serão descartadas ao sair ou recarregar.");
        }} className="min-h-11 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand-dark">Salvar preferências</button>
