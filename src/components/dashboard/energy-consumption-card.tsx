...
export function EnergyConsumptionCard({ data }: EnergyConsumptionCardProps) {
  ...
  return (
    <section aria-labelledby="energy-title" className="min-w-0 rounded-2xl border border-outline bg-white p-5 shadow-sm xl:col-span-2">
          ...
          <ul aria-label="Consumo de energia por dia" className="grid h-64 min-w-0 flex-1 gap-2 sm:gap-4" style={{ gridTemplateColumns: `repeat(${data.length}, minmax(0, 1fr))` }}>
            {data.map(({ day, kwh }) => (
              <li key={day} aria-label={`${day}: ${kwh} kWh`} className="grid min-w-0 grid-rows-[1fr_2rem]">
                <div aria-hidden="true" className="flex items-end justify-center border-b border-outline bg-linear-to-t from-brand-light/40 to-transparent">
                  <div className="relative w-full max-w-12 rounded-t-md bg-brand" style={{ height: `${(kwh / scaleMax) * 100}%` }}>
                    <span className="absolute inset-x-0 top-2 text-center text-xs font-semibold text-white tabular-nums">{kwh}</span>
                  </div>
                </div>
